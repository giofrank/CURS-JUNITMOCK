...
public class ClienteController {
	...
	public ResponseEntity<?> findAll() throws ServiceException {
		List<ClienteDomain> lista = clienteService.findByObjects(null);
		if (lista.isEmpty()) {
	...
	public ResponseEntity<?> findById(@PathVariable Integer id) throws ServiceException {
		Optional<ClienteDomain> objecto = clienteService.findById(id);
		if (objecto.isEmpty()) {
			return ResponseEntity.noContent().build();
		}
	...
	@PostMapping 
	public ResponseEntity<?> save( @RequestBody @Valid ClienteDTO dto, BindingResult bindingResult ) throws ServiceException {
	...
	@PutMapping("/{id}")
	public ResponseEntity<?> update(@PathVariable Integer id, @RequestBody @Valid ClienteDTO dto,  BindingResult bindingResult)
			throws ServiceException {
		if (bindingResult.hasErrors()) {			
		    return ResponseEntity.badRequest().body("Errores de validación: " + bindingResult.getAllErrors());
		}
		dto.setId(id);
		ClienteDTO objeto = clienteService.update(dto);
		if (Objects.isNull(objeto)) {
			return ResponseEntity.badRequest().build();
		}
